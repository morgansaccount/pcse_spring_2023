...
int main(int argc,char **argv) {
   ...
   * Allocate space.
   */
  int my_number_of_elements = rand() % nprocs, my_first_index=0;
  /*
   * Exercise:
   * - determine the index of your first element.
   *   Which routine do you use here?
   */
  MPI_Exscan(&my_number_of_elements,&my_first_index,1,MPI_INT,MPI_SUM,comm);
  ...
  MPI_Reduce(&my_number_of_elements,&total_number_of_elements,1,MPI_INT,MPI_SUM,0,comm);
  if (procno==0) {
  ...
  MPI_Gather(&my_first_index,1,MPI_INT,displ_buffer,1,MPI_INT,0,comm);
